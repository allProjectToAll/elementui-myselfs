...
export default {
  ...
  methods: {
    ...
    deletListFn() {
      ...
      //             this.postAjax("/Role/Delete",{id:id},function(res){
    ...
    allotListFn() {
      //分配角色权限
      if (this.multipleSelection.length > 1) {
        this.openErr(this.$t("message.OnlyOneDataAssignPermissions"), 1);
      } else if (this.multipleSelection.length == 0) {
        this.openErr(this.$t("message.PleaseChooseOptionsToAllocate"), 1);
      } else {
      ...
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          this.postAjax(
            ...
            this.dialogForm.addFieldForm,
            function(res) {
              console.log(that.multipleSelection[0].name);
              that.fullscreenLoading = false;
