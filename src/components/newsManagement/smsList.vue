<template>
    <div id="listContain"  class="app-container">
        <h2>{{ $t("message.smsListTitle") }}</h2><span class="commentary">&nbsp;&nbsp;{{ $t("message.Remarks") }}</span>
        <div class="clear"></div>
            ...
            <div  class="allTypeBox">
                ...
                <el-table ref="multipleTable"  stripe @selection-change="handleSelectionChange" :data="list"
                  v-loading.body="listLoading" :element-loading-text="$t('message.HardLoading')" border highlight-current-row fit  style="width: 100%">
                    ...
                    <el-table-column align="left" :label="$t('message.PhoneCode')" width="">
                        <template slot-scope="scope">
                            <span>{{scope.row.phoneCode}}</span>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column align="left" :label="$t('message.SMSContent')" width="">
                        <template slot-scope="scope">
    ...
    methods:{
        getList(){
            ...
            let par = {
             ...
             this.postAjax("/NoticeSms/GetSmsPageList",par,function(res){
