                ...
                <el-table ref="rowClick" stripe :data="list" 
                  v-loading.body="listLoading" :element-loading-text="$t('message.HardLoading')" border highlight-current-row fit  style="width: 100%">
                    ...
                    <el-table-column align="left" :label="$t('message.handle')" width="150">
                        <template slot-scope="scope">
                            <el-button size="small" type="primary"  v-if="IsInArray(btnArray,4)"  @click="reviceListFn(scope.row)">{{ $t("message.Revise") }}</el-button>
                            <el-button size="small" type="danger"  v-if="IsInArray(btnArray,4)"  @click="deletListFn(scope.row)">{{ $t("message.Delet") }}</el-button>
                        </template>
                    </el-table-column>
                </el-table>
            <!-- 分页 -->
    ...
    methods:{
        ...
        deletListFn(item){
            this.$confirm(this.$t("message.SureToDelet"), this.$t("message.Title"), {
                ...
                this.postAjax("/NoticeTpl/Delete",{"id":item.id},(res)=>{
